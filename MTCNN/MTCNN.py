...
def create_Pnet(weight_path):
    ...
    x = Conv2D(10, (3, 3), strides=1, padding='valid', name='conv1')(img_input)
    ...
    model = Model([img_input],[classifier,bbox_regress])
...
def create_Rnet(weight_path):
    ...
    # 24,24,3 -> 11,11,28
    x = Conv2D(28, (3, 3), strides=1, padding='valid', name='conv1')(img_input)
    x = PReLU(shared_axes=[1, 2], name='prelu1')(x)
...
def create_Onet(weight_path):
    ...
    # 48,48,3 -> 23,23,32
    x = Conv2D(32, (3, 3), strides=1, padding='valid', name='conv1')(img_input)
    x = PReLU(shared_axes=[1,2],name='prelu1')(x)
    x = MaxPool2D(pool_size=3, strides=2, padding='same')(x)
    # 23,23,32 -> 10,10,64
    ...
    model = Model([img_input], [classifier, bbox_regress, landmark_regress])
    model.load_weights(weight_path, by_name=True)
...
class MTCNN():
    def __init__(self):
        self.Pnet = create_Pnet(apath+'/model_data/pnet.h5')
        self.Rnet = create_Rnet(apath+'/model_data/rnet.h5')
        self.Onet = create_Onet(apath+'/model_data/onet.h5')
